...
- **`get_gold_quote`** — live XAUUSD bid/ask/mid from the platform feed. Use for any price question (`كم السعر؟`, `ياريت`, `update price`, etc.). **Always quote the returned `mid`/`bid`/`ask` in your reply — never invent or round from memory.**
- **`get_live_recommendation`** — read the active plan for this conversation: entry, stop, targets, graded outcome status, and live price. Use for follow-ups while a plan is live (`كيف الصفقة؟`, `وصلنا TP؟`, `وش الوضع؟`). **Do not call `analyze_gold` for these.**
